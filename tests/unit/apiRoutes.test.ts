...
// dataset (data/processed/demo — the fictional school, which has reviews) — no server. Asserts status codes, error shape, cache headers and the
...
beforeAll(async () => {
  ...
  const all = await repo.getProfessors('demo');
  const details = await Promise.all(all.map((p) => repo.getProfessorBySlug('demo', p.slug)));
...
describe('GET /api/health and /api/schools', () => {
  it('health mirrors meta.json', async () => {
    ...
    expect(['demo', 'live']).toContain(body.mode); // /api/health reports the default (first enabled) school
  ...
  it('schools lists every enabled school with a dataset, demo included', async () => {
    const body = await json<SchoolsResponse>(await schools(req('/api/schools')));
    expect(body.schools.map((s) => s.id)).toContain('demo');
  });
});
...
describe('GET /api/schools/[school]/subjects', () => {
  it('returns only subjects with ≥ 1 professor and accepts an upper-cased school segment', async () => {
    const body = await json<SubjectsResponse>(await subjects(req('/x'), ctx({ school: 'DEMO' })));
...
describe('GET /api/schools/[school]/rankings', () => {
  it('400 on a missing or malformed subject', async () => {
    const missing = await rankings(req('/api/schools/demo/rankings'), ctx({ school: 'demo' }));
    expect(missing.status).toBe(400);
    expect((await errorOf(missing)).code).toBe('bad_query');
    const bad = await rankings(req('/api/schools/demo/rankings?subject=CS&sort=nope'), ctx({ school: 'demo' }));
    expect(bad.status).toBe(400);
  });

  it('404 on an unknown subject; 404 on unknown school beats a valid query', async () => {
    expect((await rankings(req('/x?subject=ZZZ'), ctx({ school: 'demo' }))).status).toBe(404);
  ...
  it('applies the query server-side and echoes it', async () => {
    const res = await rankings(req('/x?subject=cs&sort=gpa&open=0&course=225'), ctx({ school: 'demo' }));
    expect(res.status).toBe(200);
...
describe('GET /api/schools/[school]/courses/[subject]/[number]', () => {
  it('returns a course-scoped RankingsResponse', async () => {
    const res = await course(req('/x?open=0'), ctx({ school: 'demo', subject: 'cs', number: '225' }));
    expect(res.status).toBe(200);
    const body = await json<RankingsResponse>(res);
    expect(body.scope).toEqual({ kind: 'course', courseId: 'demo:CS:225' });
  ...
  it('404 for unknown / malformed course segments, 400 for a bad query', async () => {
    expect((await course(req('/x'), ctx({ school: 'demo', subject: 'CS', number: '999' }))).status).toBe(404);
    expect((await course(req('/x'), ctx({ school: 'demo', subject: 'C', number: '225' }))).status).toBe(404);
    expect((await course(req('/x'), ctx({ school: 'demo', subject: 'CS', number: '22' }))).status).toBe(404);
    expect((await course(req('/x?open=2'), ctx({ school: 'demo', subject: 'CS', number: '225' }))).status).toBe(400);
  });
});

describe('GET /api/schools/[school]/professors/[slug]', () => {
  it('returns the ProfessorDetail for a known slug', async () => {
    const res = await professor(req('/x'), ctx({ school: 'demo', slug: KNOWN_SLUG }));
    expect(res.status).toBe(200);
  ...
  it('404 for unknown and malformed slugs', async () => {
    expect((await professor(req('/x'), ctx({ school: 'demo', slug: 'nobody-here' }))).status).toBe(404);
    expect((await professor(req('/x'), ctx({ school: 'demo', slug: 'bad slug!' }))).status).toBe(404);
  });
});
...
describe('GET /api/schools/[school]/professors/[slug]/summary', () => {
  it('returns a summary for a reviewed professor and says whether it was cached', async () => {
    const res = await summary(req('/x'), ctx({ school: 'demo', slug: KNOWN_SLUG }));
    expect(res.status).toBe(200);
  ...
  it('too_few_reviews when reviewCount < 3', async () => {
    const body = await json<SummaryResponse>(await summary(req('/x'), ctx({ school: 'demo', slug: LOW_DATA_SLUG })));
  ...
  it('computes the extractive summary at request time when nothing is cached', async () => {
    const detail = (await getRepository().getProfessorBySlug('demo', KNOWN_SLUG)) as ProfessorDetail;
    const uncached: ProfessorDetail = {
      ...detail, summary: null,
      professor: { ...detail.professor, id: 'demo:p:not-in-any-cache' },
      reviews: detail.reviews.map((r) => ({ ...r, professorId: 'demo:p:not-in-any-cache' })),
    };
  ...
  it('404 for an unknown professor', async () => {
    expect((await summary(req('/x'), ctx({ school: 'demo', slug: 'nobody-here' }))).status).toBe(404);
  });
});

describe('GET /api/schools/[school]/sections', () => {
  it('400 without subject, 404 for an unknown subject', async () => {
    expect((await sections(req('/x'), ctx({ school: 'demo' }))).status).toBe(400);
    expect((await sections(req('/x?subject=ZZZ'), ctx({ school: 'demo' }))).status).toBe(404);
  });

  it('returns term, seatsFetchedAt and the subject\'s sections', async () => {
    const body = await json<SectionsResponse>(await sections(req('/x?subject=cs'), ctx({ school: 'demo' })));
    expect(body.term).toMatch(/^\d{4}-(wi|sp|su|fa)$/);
    expect(body.seatsFetchedAt).toMatch(/^\d{4}-\d{2}-\d{2}T/);
    expect(body.sections.length).toBeGreaterThan(0);
    expect(body.sections.every((s) => s.courseId.startsWith('demo:CS:') || s.crossListedCourseIds.some((c) => c.startsWith('demo:CS:')))).toBe(true);
  });
});

describe('GET /api/schools/[school]/match-report', () => {
  it('returns the whole report and filters entries by method', async () => {
    const full = await json<MatchReport>(await matchReport(req('/x'), ctx({ school: 'demo' })));
    expect(full.entries.length).toBeGreaterThan(0);
    const filtered = await json<MatchReport>(await matchReport(req('/x?method=exact'), ctx({ school: 'demo' })));
  ...
  it('400 for an unknown method', async () => {
    const res = await matchReport(req('/x?method=guess'), ctx({ school: 'demo' }));
...
describe('path safety', () => {
  ...
  it('professor + summary routes: 404 for traversal / prototype / oversized slugs, body never contains the input', async () => {
    for (const slug of hostile) {
      for (const route of [professor, summary]) {
        const res = await route(req('/x'), ctx({ school: 'demo', slug }));
  ...
  it('course segments: traversal, prototype keys and oversized values are 404', async () => {
    for (const [subject, number] of [['../CS', '225'], ['CS', '../225'], ['constructor', '225'], ['CS', 'x'.repeat(500)], ['%2e%2e', '225']]) {
      const res = await course(req('/x'), ctx({ school: 'demo', subject, number }));
      expect(res.status).toBe(404);
  ...
  it('rankings / sections query: traversal in ?subject= is a 400 whose message describes the rule, not the input', async () => {
    for (const q of ['../CS', '%2e%2e%2fCS', 'constructor', 'C'.repeat(1_000)]) {
      const res = await rankings(req(`/x?subject=${encodeURIComponent(q)}`), ctx({ school: 'demo' }));
      expect(res.status).toBe(400);
      const message = (await errorOf(res)).message;
      expect(message).toMatch(/^subject:/);
      expect(message).not.toContain('..');
      expect((await sections(req(`/x?subject=${encodeURIComponent(q)}`), ctx({ school: 'demo' }))).status).toBe(400);
    }
  });
});
