...
const chromeBySchool: Record<string, SchoolChrome> = {
  ...
  purdue: { ...fallbackChrome("purdue", "Purdue"), builtAt: "2026-09-03T14:12:00Z" },
};
...
describe("buildSchoolChrome", () => {
  ...
  it("falls back to registry identity when nothing is built", () => {
    ...
    expect(c.mode).toBe("live");
    expect(c.reviewsAvailable).toBe(false);
    expect(c.seatStatusAvailable).toBe(false);
  });

  it("reads an older school.json (no flags) as reviews-available when a review adapter id is present", () => {
    const old = { id: "fp", shortName: "FPU", sources: { grades: "fp-grades", schedule: "fp-schedule", reviews: "fp-reviews" } };
    const c = buildSchoolChrome("fp", old as unknown as School, null);
    expect(c.mode).toBe("live");
...
describe("SchoolChromeBadge / SchoolProvenance follow the pathname (design §8)", () => {
  it("shows the DataBadge of the pathname's school", () => {
    pathname = "/s/uiuc/CS";
    ...
    expect(screen.getByTestId("data-badge").textContent).toContain("UIUC");
    expect(screen.queryByText(/DEMO DATA/)).toBeNull();
    cleanup();

    pathname = "/p/purdue/abad-jason";
    render(<SchoolChromeBadge chromeBySchool={chromeBySchool} defaultSchoolId="uiuc" />);
    expect(screen.getByTestId("data-badge").textContent).toContain("Purdue");
  });

  it("renders the provenance of the pathname's school, defaulting on /about", () => {
    pathname = "/compare/purdue?p=a,b";
    render(<SchoolProvenance chromeBySchool={chromeBySchool} defaultSchoolId="uiuc" />);
    expect(screen.getByTestId("data-provenance").textContent).toContain("Grades: Purdue official grade records");
    expect(screen.getByTestId("data-provenance").textContent).toContain("Reviews: none (official grade data only)");
    cleanup();
