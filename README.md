...
The deployed app never calls an upstream source, and calls an LLM only when the operator opts in with `SUMMARY_ON_DEMAND=1` **and** the request carries the shared secret `SUMMARY_ON_DEMAND_TOKEN` (see [Security](#security)). Everything else is computed once, committed, and diffed in CI.
...
ProfPeek publishes only data it is entitled to hold: official per-instructor grade aggregates released as Illinois public records (the UIUC GPA dataset declares no licence — the numbers are uncopyrightable public facts, attributed to UIUC and the curator), schedule facts from the public Course Explorer API fetched politely and cached per term, and — in future — reviews collected on ProfPeek itself under its own terms. RateMyProfessors data is never ingested into a public deployment: the adapter is a local-only, off-by-default research tool, and the product links out with plain-text attribution instead. Everything ProfPeek itself says about a real instructor (rankings, badges, AI summaries, a name match) is treated as ProfPeek's own speech, with small-cell suppression, match-tier disclosure and a correction channel as the controls.
