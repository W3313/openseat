...
flowchart LR
  subgraph Sources["Upstream sources"]
    ...
    RMP["RateMyProfessors<br/>(opt-in, live only)"]
    SEED["scripts/seed-demo.ts<br/>(fictional, seeded PRNG)"]
  end
  ...
  subgraph Adapters["src/lib/sources (GradeSource · ScheduleSource · ReviewSource)"]
    ...
    A4["DemoGradeSource · DemoScheduleSource · DemoReviewSource"]
  end
...
| Pipeline | `scripts/` | `seed-demo`, `fetch-uiuc-gpa`, `fetch-uiuc-schedule`, `ingest`, `build-rankings`, `precompute-summaries`. |
...
- **Determinism as a test.** The seed uses one `mulberry32` stream, sorted iteration and `stableStringify`; CI regenerates the data and fails on `git diff -- data`. That makes every committed number reproducible from source.
...
- **Fictional demo people.** Because the join is by name, fabricated reviews may never sit next to a real instructor. Demo mode fabricates the people too; the registry refuses to mix modes.
