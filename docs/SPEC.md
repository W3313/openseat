...
- Real sources: UIUC GPA dataset CSV (`wadefagen/datasets`, MIT-licensed) and UIUC Course Explorer public XML API. Real reviews only via an env-gated unofficial RateMyProfessors adapter (user's ToS responsibility; never runs in CI or demo).
...
- Every page shows `SiteHeader` (logo/wordmark "ProfPeek", nav: About, GitHub link, `ModeBadge`) and `SiteFooter` (`DataProvenance`: "Grades: UIUC GPA dataset (MIT) · Schedule: UIUC Course Explorer · Reviews: fictional demo data (seed 20260903) · Built {meta.builtAt} · {counts}").
...
`robots.ts`: allow all, `sitemap: {NEXT_PUBLIC_SITE_URL}/sitemap.xml`. `sitemap.ts`: `/`, `/about`, every subject page, every course page, every professor page from `subjects.json`, `courses.json`, `professors.json`.

---

## 4. API routes

All routes are `GET`, JSON, under `src/app/api/`. They call the same `Repository` and pure functions the pages use — they exist so the README can show `curl` examples, so `CompareTable`/`ShortlistDrawer` can fetch, and so a future client (or DB swap) has a stable contract. **No route ever calls Claude or an upstream source at request time.** Every response sets `Cache-Control: public, max-age=300, s-maxage=86400, stale-while-revalidate=604800`. Errors use `{ error: { code: string; message: string } }` with HTTP 400 (bad query) or 404 (unknown school/subject/professor). Route handler signature (Next 16): `export async function GET(req: Request, ctx: { params: Promise<{ school: string }> })`.
...
`src/lib/ai/openaiCompatible.ts` implements the same contract as 9.5 against any OpenAI-compatible `/chat/completions` endpoint (Groq free tier by default; Ollama, OpenRouter via `GROQ_BASE_URL`): JSON mode (`response_format: json_object`), the JSON Schema of `SummarySchema` appended to the user message, `validateOutput` (schema + evidence ids), one retry after 10 s on HTTP 429, 60 s timeout, usage totals. `SummarySource` gains `'openai-compatible'`; `ProfessorSummary.provider` carries the display name; `MetaCounts.summariesOpenAiCompatible` counts them. `resolveSummaryProvider()` (auto: Claude → OpenAI-compatible → extractive) picks the path; `isCacheHit` accepts the Claude, provider, or extractive hash. `SUMMARY_ON_DEMAND=1` lets the summary route generate (never persist) at request time; default off.
...
├── .env.example  next.config.ts  vitest.config.ts  tsconfig.json  eslint.config.mjs  postcss.config.mjs  package.json  README.md  LICENSE (MIT)
...
| `RMP_AUTH_HEADER` | `Basic dGVzdDp0ZXN0` | RmpReviewSource | Authorization header value. |
...
14. **Tech stack** badges + `LICENSE` (MIT).
...
| 14 | Claude cost/latency/outage in a public demo. | Summaries precomputed once and committed; runtime never calls Claude; API route falls back to extractive; CI runs keyless; `MAX_SUMMARIES` guard and token estimate in the script (§9.7–9.8). |
